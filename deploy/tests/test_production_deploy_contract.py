...
class ProductionDeployContractTests(unittest.TestCase):
    def test_ssm_document_is_restricted_and_self_diagnosing(self) -> None:
        ...
        self.assertIn("FISORA_LEGACY_PATH_BEFORE", commands)
        self.assertIn("FISORA_LEGACY_PATH_AFTER", commands)
        self.assertIn('git -c safe.directory="$ROOT_DIR" -C "$ROOT_DIR" restore --source=HEAD --worktree', commands)
        self.assertIn('chown ubuntu:ubuntu "$ROOT_DIR/.github" "$ROOT_DIR/.github/workflows" "$ROOT_DIR/$workflow_path"', commands)
        self.assertIn('chown -R ubuntu:ubuntu "$ROOT_DIR/.git"', commands)
        self.assertNotIn('chown -R ubuntu:ubuntu "$ROOT_DIR"', commands)
        self.assertNotIn('chown -R ubuntu:ubuntu /opt/fisora/app', commands)
        self.assertIn("FISORA_GIT_METADATA_DRIFT", commands)
        self.assertIn("FISORA_DEPLOY_REPAIRED reason=git_metadata_ownership", commands)
