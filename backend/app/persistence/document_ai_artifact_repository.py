...
class PostgresDocumentAiArtifactRepository:
    ...
    _SELECT_COLUMNS = """
        ...
        source_file_sha256, artifact_kind, stage, status, pipeline_version,
    ...
    def _from_row(row: tuple[object, ...]) -> DocumentAiArtifact:
        return DocumentAiArtifact(
            ...
            parent_artifact_id=str(row[12]) if row[12] else None,
            retry_of_artifact_id=str(row[13]) if row[13] else None,
            provider_receipt_artifact_id=str(row[14]) if row[14] else None,
            component_receipt_artifact_ids=tuple(str(value) for value in (row[15] or ())),
            expanded_from_receipt_id=str(row[16]) if row[16] else None,
            provider=str(row[17]) if row[17] else None,
            model_alias=str(row[18]) if row[18] else None,
            resolved_model=str(row[19]) if row[19] else None,
            prompt_version=str(row[20]) if row[20] else None,
            schema_version=str(row[21]) if row[21] else None,
            mapper_version=str(row[22]) if row[22] else None,
            elapsed_ms=int(row[23]) if row[23] is not None else None,
            http_status=int(row[24]) if row[24] is not None else None,
            started_at=row[25],
            finished_at=row[26],
            token_usage=dict(row[27] or {}),
            error_metadata=dict(row[28] or {}),
            metadata=dict(row[29] or {}),
            content_storage_path=str(row[30]) if row[30] else None,
            content_sha256=str(row[31]) if row[31] else None,
            request_storage_path=str(row[32]) if row[32] else None,
            request_sha256=str(row[33]) if row[33] else None,
            response_storage_path=str(row[34]) if row[34] else None,
            response_sha256=str(row[35]) if row[35] else None,
        )
