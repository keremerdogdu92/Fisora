...
alter table invoice_lines add column if not exists extraction_version integer not null default 1;
alter table invoice_lines add column if not exists source_fingerprint text;
alter table invoice_lines add column if not exists superseded_at timestamptz;
