...
**Completed 2026-09-12: REV-L05** - Date presentation is now standardized without rewriting canonical backend values: business/date-only values use `DD.MM.YYYY`, operational timestamps use `DD.MM.YYYY · HH:mm` in `Europe/Istanbul`, accounting periods keep their existing period semantics, and context-specific dashboard activity remains intentionally time-only. Shared formatters validate canonical/known day-first values and preserve unknown strings instead of guessing. Workbench, client document, QNB, Research, Learned Rules, Operations/session, and statement-review timestamp surfaces now reuse that layer. Acceptance: formatter 3/3, portal-next targeted 23/23, targeted queue Chromium 1/1, frontend 239/239, document-inspector Chromium 12/12, TypeScript/build PASS, mojibake guard PASS, `git diff --check` PASS.
