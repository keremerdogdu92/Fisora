...
class QnbIncomingSafetyTests(unittest.TestCase):
    ...
    def test_production_config_has_single_qnb_owner_and_tls_preflight(self) -> None:
        ...
        self.assertIn("Live data/QNB mode requires HTTPS", preflight)
        self.assertIn("fullchain.pem", preflight)
        self.assertIn('if [ "$qnb_scheduler" = "true" ]; then', preflight)
        self.assertIn("QNB scheduler requires FISORA_QNB_CREDENTIAL_KEY", preflight)
