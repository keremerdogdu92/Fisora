...
class ThreeStageAccountingPipelineTests(unittest.TestCase):
    ...
    def test_ttnet_preserves_current_invoice_basis_and_planner_owned_current(self) -> None:
        ...
        self.assertEqual(result["draft_lines"][0]["source_position"], "1")
        self.assertEqual(result["draft_lines"][0]["source_text"], READER["invoice_table_rows"][0]["source_text"])
        self.assertEqual(result["draft_lines"][1]["source_line_numbers"], [8])
        self.assertEqual(result["draft_lines"][1]["source_position"], "8")
        self.assertEqual(result["draft_lines"][1]["source_text"], READER["invoice_table_rows"][1]["source_text"])
        decision_ids = {item["source_position"]: item["canonical_line_id"] for item in result["line_decisions"]}
