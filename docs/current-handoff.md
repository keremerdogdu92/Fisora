...
- The SSM worktree guard does not auto-reset unknown tracked changes. It emits `FISORA_DEPLOY_BLOCKED reason=tracked_worktree_changes`, includes the changed paths and workflow blob evidence, and exits safely. The only worktree self-repair exception is the known OIDC-transition workflow blob `7001c451a720f682b5334f97d10dab9643c38180`; it is restored from HEAD only when `.github/workflows/deploy-production.yml` is the sole tracked drift. That repair normalizes ownership only for `.github`, `.github/workflows`, and the workflow file to `ubuntu:ubuntu`. A separate preflight detects root-owned Git metadata left by the historical root-context SSM Git commands and, when present, normalizes only `$ROOT_DIR/.git` recursively to `ubuntu:ubuntu`; the application worktree is never recursively chowned. Both repairs emit before/after evidence and explicit `FISORA_DEPLOY_REPAIRED` reasons.
