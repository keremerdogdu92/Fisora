...
Bilinmeyen tracked production worktree değişikliklerinde deploy otomatik reset atmaz. `FISORA_DEPLOY_BLOCKED reason=tracked_worktree_changes` ile değişen dosyaları loglar ve güvenli şekilde durur. Yalnızca OIDC geçişinden kalmış, blob hash'i `7001c451a720f682b5334f97d10dab9643c38180` olan eski `.github/workflows/deploy-production.yml` sürümü tek kirli tracked dosyaysa HEAD sürümüne geri alınır. Bu tek seferlik repair sırasında yalnız `.github`, `.github/workflows` ve workflow dosyasının sahipliği `ubuntu:ubuntu` olarak normalize edilir; recursive `chown` yapılmaz. Önce/sonra metadata ve `FISORA_DEPLOY_REPAIRED reason=known_legacy_workflow_blob` kaydı loglanır.
